...
(defn output-main-js-file-shadow [output-to main-ns]
  (let [http-port (utils/server-port utils/http-server)
        http-host (utils/server-host utils/http-server)]
    (io/make-parents output-to)
    (spit
...
(function() {
  ...
  var host = '%s';
  %s
  document.write('<script src=\"http://' + host + ':' + port + '/shadow_repl_main.js\"></script>');
  %s})();"
             http-port http-host
             (if main-ns (str "var mainNs = '" (namespace-munge main-ns) "';") "")
             (if main-ns
               "document.write('<script>replique.cljs_env.repl.connect(\"http://' + host + ':' + port + '\" , \"' + mainNs + '\");</script>');"
               "")))))
...
(defmethod tooling-msg/tooling-msg-handle [:replique/shadow-browser :output-main-js-files]
  [{:keys [output-to main-ns] :as msg}]
  (tooling-msg/with-tooling-response msg
    (output-main-js-file-shadow output-to main-ns)
    msg))
