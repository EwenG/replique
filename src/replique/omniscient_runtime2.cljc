(ns replique.omniscient-runtime2
        ...
        (if k
          (let [placeholder (make-array Integer 0)
            ...
            (when (not (identical? env-v placeholder))
              (let [env-v (if (= :thread k) (.getName ^Thread env-v) env-v)]
...
(defn format-env-map-step [acc k v]
  (assoc acc (str k) (pr-str v)))

(defn format-env-step [acc k v]
  (case k
    :locals (assoc acc k (reduce-kv format-env-map-step {} v))
    :bindings (assoc acc k (reduce-kv format-env-map-step {} v))
    :thread (assoc acc k (.getName ^Thread v))
    :index (assoc acc k v)
    (assoc acc k (str v))))

(defn format-env [env]
  (reduce-kv format-env-step {} env))
...
(defmulti filter-env (fn [qualified-sym env] qualified-sym))

(defmethod filter-env :default [_ _] true)

(defn filter-envs-m [qualified-sym]
  (let [envs (get @registry qualified-sym)]
    (into [] (filter (partial filter-env qualified-sym)) envs)))
