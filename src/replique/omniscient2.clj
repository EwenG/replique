                        ...
                        :bindings (->> (get-thread-bindings)
                                       (map (fn [[k# v#]] (when-let [sym# (var->sym nil k#)]
                                                            (when (not
                                                              ...
                                                              [sym# v#]))))
                                       (into {}))
                        ;; A uuid, useful to distinguish two values of a same var (accros multiple
                        ;; redefinitions of the same var) 
                        :value-uuid ~uuid})))
...
(defn eval-with-env-cljs [repl-env env form opts]
  (if (= (utils/repl-ns :replique/cljs) (namespace *omniscient-sym*))
    (let [form (-> form
                   (form-with-locals *omniscient-local-syms*)
                   (form-with-bindings *omniscient-binding-syms*))]
...
#_(defmethod tooling-msg/tooling-msg-handle [:replique/clj :omniscient-filter]
...
(defmethod tooling-msg/tooling-msg-handle [:replique/clj :omniscient-filter]
  [{sym :symbol ns :ns is-string? :is-string? :as msg}]
  (tooling-msg/with-tooling-response msg
    (let [the-ns (and ns (find-ns (symbol ns)))
          sym (and sym (symbol sym))
          qualified-sym (compute-qualified-sym nil the-ns sym)]
      (when (and (not is-string?) qualified-sym)
        (let [filtered-envs (replique.omniscient-runtime/filter-envs-m qualified-sym)
              formatted-envs (into [] (map replique.omniscient-runtime/format-env) filtered-envs)]
          (assoc msg :envs formatted-envs))))))
