...
(defmacro capture-env []
  {:locals (locals-map (env->locals &env))
...
(defmacro with-env [env & body]
  (let [syms (get-binding-syms &env env)
        locals-syms (:locals syms)
        binding-syms (:bindings syms)]
    `(binding ~(reduce (partial bindings-reducer env) [] binding-syms)
       (let ~(reduce (partial locals-reducer env) [] locals-syms)
         ~@body))))
